            ...
            <div class="lg:col-span-2 space-y-5">
                <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required placeholder="Enter blog post title..."
                    ...
                    <div>
                        ...
                        <textarea name="excerpt" rows="2" placeholder="Brief summary shown in blog cards..."
                                  class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">{{ old('excerpt') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Content</label>
                        <textarea name="content" id="blog_content" class="w-full border rounded p-2">{{ old('content') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="author" value="{{ old('author', 'Admin') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1">Published Date</label>
                        <input type="datetime-local" name="published_at" value="{{ old('published_at', now()->format('Y-m-d\TH:i')) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1">Sort Order</label>
                        <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none">
                    </div>
                    <div class="flex flex-col gap-2">
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="featured" class="w-4 h-4 text-yellow-500 border-gray-300 rounded">
...
document.getElementById('thumbnail_input').addEventListener('change', function(e) {
    ...
    if (file) {
        ...
        reader.onload = function(ev) {
            document.getElementById('thumb_img').src = ev.target.result;
            document.getElementById('thumb_preview').classList.remove('hidden');
        };
