            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden p-1">
                                            @if ($method->logo)
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2 py-1 rounded-full text-[10px] font-bold uppercase {{ $method->type == 'merchant' ? 'bg-purple-100 text-purple-600' : ($method->type == 'agent' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600') }}">
                                        {{ $method->type }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <form action="{{ route('admin.payment-methods.toggle', $method) }}" method="POST">
                                        @csrf
                                        <button type="submit"
                                            class="flex items-center gap-1 {{ $method->status ? 'text-green-600' : 'text-red-400' }}">
                                            <i class="fas fa-toggle-{{ $method->status ? 'on' : 'off' }} text-lg"></i>
                                            <span
                                                class="text-xs font-medium">{{ $method->status ? 'Active' : 'Inactive' }}</span>
                                        </button>
                                    </form>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <button
                                            onclick="openEditModal({{ $method->id }}, '{{ addslashes($method->name) }}', '{{ $method->type }}', '{{ $method->number }}', '{{ addslashes($method->notes ?? '') }}')"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-400">No payment methods found.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="addPaymentMethodModal"
        ...
        <div class="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-fade-in">
            <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                ...
                <button onclick="document.getElementById('addPaymentMethodModal').classList.add('hidden')"
            ...
            <form action="{{ route('admin.payment-methods.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="pt-4 flex gap-3">
                    <button type="button"
                        onclick="document.getElementById('addPaymentMethodModal').classList.add('hidden')"
                    ...
                    <button type="submit"
                        class="flex-1 bg-accent-blue text-white px-4 py-2.5 rounded-lg font-bold hover:bg-blue-700 shadow-md transition-all">Add
                        Method</button>
                </div>
            </form>
        </div>
    ...
    <div id="editPaymentMethodModal"
        ...
        <div class="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-fade-in">
            <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                ...
                <button onclick="document.getElementById('editPaymentMethodModal').classList.add('hidden')"
                    class="text-gray-400 hover:text-gray-600">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <form id="editPaymentMethodForm" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                ...
                <div class="pt-4 flex gap-3">
                    <button type="button"
                        onclick="document.getElementById('editPaymentMethodModal').classList.add('hidden')"
                        class="flex-1 px-4 py-2.5 rounded-lg font-bold text-gray-500 border border-gray-200 hover:bg-gray-50 transition-all">Cancel</button>
                    <button type="submit"
