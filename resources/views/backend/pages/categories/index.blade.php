...
<div class="max-w-6xl mx-auto py-8 px-4">
    <div class="mb-8 flex justify-between items-center">
        ...
        <button onclick="document.getElementById('addCategoryModal').classList.remove('hidden')" class="bg-accent-blue text-white px-6 py-2.5 rounded-lg font-bold hover:bg-blue-700 transition-all flex items-center gap-2 shadow-lg shadow-blue-100">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($categories as $category)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors bg-white">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <button onclick="openEditModal({{ $category->id }}, '{{ addslashes($category->name) }}', '{{ $category->parent_id }}', '{{ addslashes($category->icon ?? '') }}', {{ $category->order }}, {{ $category->is_featured ? 'true' : 'false' }}, [{{ $category->brands->pluck('id')->join(',') }}])" class="text-blue-400 hover:text-blue-600 transition-colors"><i class="fas fa-edit"></i></button>
                        ...
                        @foreach($category->children as $sub)
                            <tr class="hover:bg-gray-50 transition-colors bg-gray-50/30">
                                ...
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-3">
                                        <button onclick="openEditModal({{ $sub->id }}, '{{ addslashes($sub->name) }}', '{{ $sub->parent_id }}', '{{ addslashes($sub->icon ?? '') }}', {{ $sub->order }}, {{ $sub->is_featured ? 'true' : 'false' }}, [{{ $sub->brands->pluck('id')->join(',') }}])" class="text-blue-400 hover:text-blue-600 transition-colors"><i class="fas fa-edit"></i></button>
                            ...
                            @foreach($sub->children as $subSub)
                                <tr class="hover:bg-gray-50 transition-colors bg-white">
                                    ...
                                    <td class="px-6 py-2">
                                        <div class="flex items-center gap-3">
                                            <button onclick="openEditModal({{ $subSub->id }}, '{{ addslashes($subSub->name) }}', '{{ $subSub->parent_id }}', '{{ addslashes($subSub->icon ?? '') }}', {{ $subSub->order }}, {{ $subSub->is_featured ? 'true' : 'false' }}, [{{ $subSub->brands->pluck('id')->join(',') }}])" class="text-blue-400 hover:text-blue-600 transition-colors"><i class="fas fa-edit"></i></button>
...
<!-- Edit Modal -->
<div id="editCategoryModal" class="fixed inset-0 bg-gray-900/10 backdrop-blur-[2px] z-[100] hidden flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-fade-in">
        ...
        <form id="editCategoryForm" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            ...
            <div>
                ...
                <select name="parent_id" id="edit_parent_id" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-accent-orange transition-all">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="icon" id="edit_icon" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-accent-orange transition-all" placeholder="fas fa-laptop">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Sort Order</label>
                    <input type="number" name="order" id="edit_order" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-accent-orange transition-all">
                </div>
            </div>
            <div>
                <div class="flex justify-between items-center mb-1">
                    ...
                    <div class="flex gap-2 bg-gray-50 px-2 py-0.5 rounded border border-gray-100">
                        <button type="button" onclick="$('#edit_brand_select').val($('#edit_brand_select option').map(function(){return this.value}).get()).trigger('change');" class="text-[9px] font-bold text-accent-blue hover:text-blue-700 uppercase tracking-wider">All</button>
                        <span class="text-gray-300 text-[10px]">|</span>
                        <button type="button" onclick="$('#edit_brand_select').val(null).trigger('change');" class="text-[9px] font-bold text-red-500 hover:text-red-700 uppercase tracking-wider">None</button>
                    </div>
                </div>
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_featured" id="edit_is_featured" value="1" class="w-4 h-4 text-accent-orange border-gray-300 rounded">
                <label for="edit_is_featured" class="text-sm font-semibold text-gray-700">Feature this category on home page</label>
            </div>
            <div>
            ...
            <div class="pt-4 flex gap-3">
                <button type="button" onclick="document.getElementById('editCategoryModal').classList.add('hidden')" class="flex-1 px-4 py-2.5 rounded-lg font-bold text-gray-500 border border-gray-200 hover:bg-gray-50 transition-all">Cancel</button>
                <button type="submit" class="flex-1 bg-accent-blue text-white px-4 py-2.5 rounded-lg font-bold hover:bg-blue-700 shadow-md transition-all">Update Category</button>
            </div>
        </form>
    </div>
</div>
...
<!-- Add Modal -->
<div id="addCategoryModal" class="fixed inset-0 bg-gray-900/10 backdrop-blur-[2px] z-[100] hidden flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-fade-in">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            ...
            <div class="relative">
                <div class="flex justify-between items-center mb-1">
                    <label class="block text-sm font-semibold text-gray-700">Associate Brands</label>
                    <div class="flex gap-2 bg-gray-50 px-2 py-0.5 rounded border border-gray-100">
                        <button type="button" onclick="$('#brand_select').val($('#brand_select option').map(function(){return this.value}).get()).trigger('change');" class="text-[9px] font-bold text-accent-blue hover:text-blue-700 uppercase tracking-wider">All</button>
                        <span class="text-gray-300 text-[10px]">|</span>
                        <button type="button" onclick="$('#brand_select').val(null).trigger('change');" class="text-[9px] font-bold text-red-500 hover:text-red-700 uppercase tracking-wider">None</button>
                    </div>
                </div>
                <select name="brand_ids[]" id="brand_select" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 outline-none focus:border-accent-orange transition-all select2-multiple" multiple="multiple">
                    @foreach($brands as $brand)
                        <option value="{{ $brand->id }}">{{ $brand->name }}</option>
                    @endforeach
                </select>
                <p class="text-[11px] text-gray-400 mt-2 italic">Search and select multiple brands to show in sub-menu.</p>
            </div>
