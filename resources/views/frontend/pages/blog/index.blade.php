...
@section('title', isset($category) ? $category->name . ' — Blog | Iosbd' : 'Blog & Tips | Iosbd')
...
@section('content')
    <div class="container mx-auto px-4 py-8 mb-12">
        ...
        <div class="text-center mb-10">
            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Iosbd Blog</span>
        ...
        @if (isset($featuredBlog) && !isset($category))
            <div class="mb-10">
                <a href="{{ route('blogs.show', $featuredBlog->slug) }}" class="block group">
                    <div
                        class="relative rounded-2xl overflow-hidden shadow-lg border border-gray-100 bg-white flex flex-col md:flex-row">
                        <div class="md:w-1/2 relative overflow-hidden h-52 md:h-auto">
                            ...
                            @else
                                <div
                                    class="w-full h-full bg-gradient-to-br from-blue-900 to-indigo-700 flex items-center justify-center min-h-52">
                                    <i class="fas fa-newspaper text-white/30 text-6xl"></i>
                                </div>
                            @endif
                            <span
                                class="absolute top-3 left-3 bg-[#ef4a23] text-white text-[10px] font-bold px-2 py-1 rounded">Featured</span>
                        </div>
                        <div class="md:w-1/2 p-8 flex flex-col justify-center">
                            <span
                                class="text-xs font-bold text-[#ef4a23] uppercase tracking-wider mb-2">{{ $featuredBlog->category->name ?? '' }}</span>
                            <h2
                                class="text-2xl font-extrabold text-gray-900 mb-3 leading-snug group-hover:text-[#ef4a23] transition-colors">
                            ...
                            <div class="flex items-center gap-3 text-xs text-gray-400">
                                ...
                                <span><i
                                        class="fas fa-calendar mr-1"></i>{{ $featuredBlog->published_at?->format('d M Y') }}</span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach ($blogs as $blog)
                    <a href="{{ route('blogs.show', $blog->slug) }}"
                        class="blog-card bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                        <div class="relative overflow-hidden h-44 bg-gradient-to-br from-blue-900 to-indigo-700">
                            @if ($blog->thumbnail)
                                <img src="{{ asset('storage/' . $blog->thumbnail) }}"
                                    class="blog-thumb w-full h-full object-cover" alt="{{ $blog->title }}">
                            @else
                            ...
                            @endif
                            <span
                                class="absolute top-2 left-2 bg-white/90 text-[10px] font-bold text-gray-700 px-2 py-0.5 rounded-full">
                                {{ $blog->category->name ?? '' }}
                            </span>
                            @if ($blog->featured)
                                <span
                                    class="absolute top-2 right-2 bg-[#ef4a23] text-white text-[9px] font-bold px-2 py-0.5 rounded-full">⭐
                                    Featured</span>
                            @endif
                        </div>
                        <div class="p-4 flex flex-col flex-1">
                            <h3 class="text-sm font-extrabold text-gray-900 mb-2 line-clamp-2 leading-snug">
                                {{ $blog->title }}</h3>
                            ...
                            <div
                                ...
                                <span><i
                                        class="fas fa-calendar mr-1"></i>{{ $blog->published_at?->format('d M Y') }}</span>
                            </div>
                        </div>
                    </a>
                @endforeach
        ...
        @else
            <div class="text-center py-20">
                ...
                <a href="{{ route('blogs.index') }}" class="text-[#ef4a23] font-bold text-sm mt-2 inline-block">← View All
                    Posts</a>
            </div>
        @endif
