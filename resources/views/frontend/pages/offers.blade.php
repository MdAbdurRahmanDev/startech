...
@section('title', 'Promotional Offers | Star Tech')
...
<div class="container mx-auto px-2 md:px-4 mb-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($offers as $offer)
            <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100 flex flex-col hover:shadow-xl transition-shadow duration-300">
                <!-- Offer Image -->
                <a href="{{ route('offers.show', $offer->slug) }}" class="block relative aspect-[4/3] overflow-hidden">
                    <img src="{{ asset('storage/' . $offer->image) }}" alt="{{ $offer->title }}" class="w-full h-full object-cover hover:scale-105 transition-transform duration-500">
                </a>

                <!-- Offer Info -->
                <div class="p-5 flex flex-col flex-grow">
                    <!-- Meta Info -->
                    <div class="flex items-center justify-between text-[12px] text-gray-500 mb-4 pb-4 border-b border-gray-50">
                        <div class="flex items-center gap-1.5">
                            <i class="far fa-calendar-alt text-accent-orange"></i>
                            <span>{{ $offer->start_date->format('d M Y') }} - {{ $offer->end_date->format('d M Y') }}</span>
                        </div>
                    ...
                    <!-- Title & Description -->
                    <h2 class="text-[18px] font-bold text-primary-dark mb-3 line-clamp-2 min-h-[54px]">{{ $offer->title }}</h2>
                    <p class="text-[13px] text-gray-600 mb-6 line-clamp-2 leading-relaxed">{{ $offer->short_description }}</p>

                    <!-- Action Button -->
                    <div class="mt-auto">
                        <a href="{{ route('offers.show', $offer->slug) }}" class="inline-block bg-accent-blue text-white px-8 py-2.5 rounded font-bold text-sm hover:bg-opacity-90 transition-all">
                            View Details
                        </a>
                    </div>
                </div>
