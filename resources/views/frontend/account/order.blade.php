                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-sm text-gray-700 font-medium">{{ $order->first_name }}
                                    {{ $order->last_name }}</p>
                                <p class="text-[10px] text-gray-400">{{ $order->phone }}</p>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span
                                    class="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded">{{ $order->items->count() }}</span>
                            </td>
                            <td class="px-6 py-4">
                                @php
                                    $statusClasses = [
                                ...
                                @endphp
                                <span
                                    class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border {{ $statusClasses[$order->status] ?? 'bg-gray-50 text-gray-600 border-gray-100' }}">
                                    {{ str_replace('_', ' ', $order->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span
                                    class="text-sm font-bold text-accent-orange">{{ number_format($order->total, 0) }}৳</span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('order.invoice.show', $order->id) }}"
                                        class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-accent-orange group-hover:text-white transition-all shadow-sm"
                                        title="View Invoice">
                                    ...
                                    @else
                                        <span
                                            class="px-2 py-0.5 bg-gray-100 text-gray-500 rounded text-[9px] font-bold uppercase border border-gray-200"
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    <div
                                        class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center text-gray-300 text-2xl">
                                        <i class="fas fa-shopping-bag"></i>
                                    </div>
                                    <p class="text-gray-400 text-sm font-medium">You have not made any previous orders!</p>
                                    <a href="{{ url('/') }}"
                                        class="mt-2 text-accent-blue text-xs font-bold hover:underline">Start Shopping</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
