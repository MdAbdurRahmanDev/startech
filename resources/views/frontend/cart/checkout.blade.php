...
<style>
    @media (max-width: 1023px) {
        .checkout-sidebar { flex: 1 1 100% !important; max-width: 100% !important; }
    }
</style>
@endsection
...
<div class="pb-14">
    ...
    <div class="bg-[#e8f5e9] border border-[#c8e6c9] text-[13px] text-gray-700 px-4 py-3 rounded mb-4 leading-relaxed">
        ...
        কারিগরি ত্রুটির কারণে পণ্যের মূল্য অসঙ্গতিপূর্ণ হলে, স্টার টেক কর্তৃপক্ষ অর্ডার বাতিলের অধিকার সংরক্ষণ করে। অনুগ্রহ করে কাস্টমার সাপোর্ট এজেন্টের কনফার্মেশন ব্যতীত কোনো ধরনের পেমেন্ট প্রদান না করার অনুরোধ করা হচ্ছে।
    </div>
    ...
    <form action="{{ route('order.place') }}" method="POST" id="checkoutForm">
        ...
        <div class="flex flex-col lg:flex-row gap-6 items-start">
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-6">
                        ...
                        <div class="space-y-2.5">
                            ...
                            <label class="flex items-center gap-2.5 cursor-pointer">
                                ...
                                <span class="text-[13px] text-gray-700">{{ $method->name }} - <strong>{{ number_format($method->cost, 0) }}৳</strong></span>
                            </label>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="flex items-center justify-between py-3">
                            <div>
                                <p class="text-[13px] text-gray-800">
                                    ...
                                    <a href="{{ url('product/' . $item['slug']) }}" class="text-accent-blue hover:underline ml-1">{{ $item['name'] }}</a>
                                </p>
                                <p class="text-[11px] text-gray-400 mt-0.5">Star Points: {{ round($item['price'] * $item['quantity'] / 100) }}</p>
                            </div>
            ...
            {{-- ── RIGHT SIDEBAR ── --}}
            <div class="checkout-sidebar w-full shrink-0 sticky top-4 space-y-5" style="width: 100%; max-width: 320px; flex: 0 0 320px;">
                <div class="bg-white rounded-lg shadow-sm p-5">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" class="text-accent-blue text-[12px] font-bold hover:text-accent-orange transition-colors px-1">Apply</button>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-gray-100 pt-4 space-y-2.5 text-[13px]">
                        ...
                        <div class="flex justify-between text-gray-600">
                            ...
                            <span id="delivery-charge" class="font-bold text-gray-800">{{ number_format($shipping_methods->first()->cost ?? 0, 0) }}৳</span>
                        </div>
                        <div class="border-t border-gray-100 pt-2.5 flex justify-between text-[14px] font-bold text-gray-800">
                            <span>Total:</span>
                            <span id="grand-total" class="text-accent-orange text-[15px] font-bold">{{ number_format($total + ($shipping_methods->first()->cost ?? 0), 0) }}৳</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 pt-4 border-t border-gray-100">
                        <div class="flex items-start gap-2 mb-4">
                            <input type="checkbox" id="terms" name="terms" required class="mt-0.5 accent-accent-blue">
                            <label for="terms" class="text-[11px] text-gray-600 leading-relaxed">
                                I have read and agree to the
                                <a href="#" class="text-accent-orange hover:underline font-semibold">Terms and Conditions</a>,
                                <a href="#" class="text-accent-orange hover:underline font-semibold">Privacy Policy</a> and
                                <a href="#" class="text-accent-orange hover:underline font-semibold">Refund and Return Policy</a>
                            </label>
                        </div>
                        <button type="submit"
                        ...
                        <a href="{{ route('cart.show') }}" class="mt-3 text-[12px] text-gray-400 hover:text-accent-blue transition-colors flex items-center justify-center gap-1.5">
...
<script>
    ...
    function switchCouponTab(tab) {
        ...
        if (tab === 'coupon') {
            couponBtn.className = 'flex items-center gap-1.5 bg-accent-blue text-white px-3 py-1.5 rounded text-[12px] font-semibold transition-colors';
            giftBtn.className = 'flex items-center gap-1.5 bg-gray-100 text-gray-600 px-3 py-1.5 rounded text-[12px] font-semibold transition-colors hover:bg-gray-200';
            input.placeholder = 'Promo / Coupon Code';
        } else {
            giftBtn.className = 'flex items-center gap-1.5 bg-accent-blue text-white px-3 py-1.5 rounded text-[12px] font-semibold transition-colors';
            couponBtn.className = 'flex items-center gap-1.5 bg-gray-100 text-gray-600 px-3 py-1.5 rounded text-[12px] font-semibold transition-colors hover:bg-gray-200';
