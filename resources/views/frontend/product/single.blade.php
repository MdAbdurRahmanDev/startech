...
@section('content')
    <div class="container pb-14">
        ...
        <div class="py-4 text-[13px] text-gray-600">
            ...
            @foreach ($product->categories as $cat)
                / <a href="{{ url('category/' . $cat->slug) }}"
                    class="text-gray-800 no-underline hover:text-accent-orange transition-colors">{{ $cat->name }}</a>
            @endforeach
                    ...
                    <div class="flex flex-wrap gap-2.5 mb-5">
                        <span class="bg-[#f2f4f8] py-1 px-3.5 rounded-full text-[13px] text-gray-800 inline-block">
                            ...
                            @if ($product->discount_price && $product->discount_price < $product->price)
                                ...
                                <span
                                    class="line-through text-[11px] text-gray-400 ml-1">{{ number_format($product->price, 0) }}৳</span>
                            @else
                        ...
                        @if ($product->categories->count() > 0)
                            <span
                                class="bg-[#f2f4f8] py-1 px-3.5 rounded-full text-[13px] text-gray-800 inline-block">Category:
                                <strong>{{ $product->categories->first()->name }}</strong></span>
                        @endif
                        @if ($product->tags)
                            <span class="bg-[#f2f4f8] py-1 px-3.5 rounded-full text-[13px] text-gray-800 inline-block">Tags:
                                <strong>{{ $product->tags }}</strong></span>
                        @endif
                    </div>
                        ...
                        <div class="mb-5">
                            ...
                            @if ($product->specifications->count() > 5)
                                <a href="#specification"
                                    class="text-accent-orange text-[13px] no-underline mt-2 inline-block transition-colors hover:text-orange-700">
                            ...
                            <div class="flex flex-wrap items-center gap-4">
                                <div class="flex items-center border border-gray-300 rounded overflow-hidden">
                                    <button type="button" onclick="changeQty(-1)"
                                        class="px-3.5 py-2 bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer text-[18px] leading-none text-gray-700">−</button>
                                    <input type="number" id="qty" value="1" min="1"
                                        max="{{ $product->stock }}"
                                        class="w-[50px] text-center border-x border-gray-300 py-2.5 outline-none text-[15px] font-semibold text-gray-800 appearance-none m-0">
                                    <button type="button" onclick="changeQty(1)"
                                        class="px-3.5 py-2 bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer text-[18px] leading-none text-gray-700">+</button>
                                </div>
                                <button onclick="buyNow({{ $product->id }})"
                                    class="bg-accent-blue text-white py-3 px-8 rounded font-bold flex-grow text-center transition-colors hover:bg-accent-orange shadow-sm flex items-center justify-center gap-2">
                                    <i class="fas fa-shopping-cart"></i> Buy Now
                                </button>
                                <button onclick="addToCart({{ $product->id }})"
                                    class="bg-gray-800 text-white py-3 px-8 rounded font-bold flex-grow text-center transition-colors hover:bg-accent-orange shadow-sm flex items-center justify-center gap-2">
        ...
        <div class="mt-10">
            <nav class="section-nav flex gap-2.5 mb-5 flex-wrap">
                <a href="#specification"
                    class="active bg-white text-gray-800 py-2.5 px-5 rounded text-[14px] font-semibold shadow-[0_1px_3px_rgba(0,0,0,0.05)] transition-colors hover:bg-accent-orange hover:text-white [&.active]:bg-accent-orange [&.active]:text-white">Specification</a>
                @if ($product->description)
                    <a href="#description"
                        class="bg-white text-gray-800 py-2.5 px-5 rounded text-[14px] font-semibold shadow-[0_1px_3px_rgba(0,0,0,0.05)] transition-colors hover:bg-accent-orange hover:text-white [&.active]:bg-accent-orange [&.active]:text-white">Description</a>
                @endif
                @if ($product->video)
                    <a href="#video"
                        class="bg-white text-gray-800 py-2.5 px-5 rounded text-[14px] font-semibold shadow-[0_1px_3px_rgba(0,0,0,0.05)] transition-colors hover:bg-accent-orange hover:text-white [&.active]:bg-accent-orange [&.active]:text-white">Product
                        Video</a>
                @endif
                <a href="#questions"
                    class="bg-white text-gray-800 py-2.5 px-5 rounded text-[14px] font-semibold shadow-[0_1px_3px_rgba(0,0,0,0.05)] transition-colors hover:bg-accent-orange hover:text-white [&.active]:bg-accent-orange [&.active]:text-white">Questions
                    (1)</a>
                <a href="#reviews"
                    class="bg-white text-gray-800 py-2.5 px-5 rounded text-[14px] font-semibold shadow-[0_1px_3px_rgba(0,0,0,0.05)] transition-colors hover:bg-accent-orange hover:text-white [&.active]:bg-accent-orange [&.active]:text-white">Reviews
                    (1)</a>
            </nav>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5 items-start">
                ...
                <div>
                    ...
                    @if ($product->description)
                        <div id="description"
                            class="content-card bg-white rounded-lg shadow-sm p-6 mb-5 text-[14px] leading-relaxed text-gray-700 [&>h3]:text-[16px] [&>h3]:my-4 [&>h3]:text-gray-800 [&>p]:mb-4 last:[&>p]:mb-0">
                            <h2 class="text-[18px] font-bold text-gray-800 mb-4 border-b border-gray-100 pb-3">Description
                            </h2>
                    ...
                    @if ($product->video)
                        <div id="video" class="content-card bg-white rounded-lg shadow-sm p-6 mb-5">
                            <h2 class="text-[18px] font-bold text-gray-800 mb-4 border-b border-gray-100 pb-3">Product
                                Video</h2>
                            <div
                                class="aspect-video w-full rounded-lg overflow-hidden bg-black border border-gray-100 shadow-inner">
                    ...
                    <div class="content-card bg-white rounded-lg shadow-sm p-6 mb-5">
                        <h2 class="text-[18px] font-bold text-gray-800 mb-4">What is the price of {{ $product->name }} in
                            Bangladesh?</h2>
                        <p class="text-[14px] text-gray-600 leading-relaxed">
                            The latest price of {{ $product->name }} in Bangladesh is
                            {{ number_format($product->discount_price ?? $product->price, 0) }}৳. You can buy the
                            {{ $product->name }} at best price from our website or visit any of our showrooms.
                        </p>
                    </div>
                    ...
                    <div id="questions" class="content-card bg-white rounded-lg shadow-sm p-6 mb-5">
                        <div
                            class="flex flex-col sm:flex-row justify-between items-start border-b border-gray-100 pb-4 mb-4 gap-4">
                            <div>
                                <h2 class="text-[18px] font-bold text-gray-800 mb-1">Questions
                                    ({{ $product->questions->where('status', 1)->count() }})</h2>
                                <p class="text-[13px] text-gray-500">Have question about this product? Get specific details
                                    about this product from expert.</p>
                            </div>
                            <button onclick="toggleQuestionModal()"
                                class="shrink-0 border border-gray-800 text-gray-800 py-2 px-4 rounded text-[13px] font-semibold transition-colors hover:bg-gray-800 hover:text-white">Ask
                                Question</button>
                        </div>

                        @forelse($product->questions->where('status', 1) as $question)
                            <div class="py-4 border-b border-[#f2f4f8] last:border-b-0">
                                <div class="text-[12px] text-gray-400 mb-2.5">{{ $question->name }} on
                                    {{ $question->created_at->format('d M Y') }}</div>
                                <div class="text-[14px] font-bold text-gray-800 mb-1.5">Q: {{ $question->question }}</div>
                                <div class="text-[14px] text-gray-600">A:
                                    {{ $question->answer ?? 'Pending expert response...' }}</div>
                            </div>
                        @empty
                    ...
                    <div id="questionModal"
                        class="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/40 backdrop-blur-sm transition-opacity">
                        <div
                            class="bg-white rounded-xl shadow-2xl w-full max-w-[500px] overflow-hidden relative mx-4 animate-scale-up">
                            <button onclick="toggleQuestionModal()"
                                class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 focus:outline-none z-10 transition-colors">
                                ...
                                <form action="{{ route('product.question.store') }}" method="POST" class="space-y-6">
                                    ...
                                    <div>
                                        <label
                                            class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Your
                                            Question</label>
                                        <textarea name="question" rows="4" required placeholder="Type your question here..."
                                            class="w-full bg-gray-50 border border-gray-200 rounded-lg py-3 px-4 focus:ring-2 focus:ring-accent-blue outline-none transition-all text-sm"></textarea>
                                    </div>

                                    <button type="submit"
                                        class="w-full bg-accent-orange text-white py-3.5 rounded-lg font-bold text-sm hover:shadow-lg transition-all flex items-center justify-center gap-2">
                    ...
                    <div id="reviews" class="content-card bg-white rounded-lg shadow-sm p-6 mb-5">
                        <div
                            class="flex flex-col sm:flex-row justify-between items-start border-b border-gray-100 pb-4 mb-4 gap-4">
                            <div>
                                <h2 class="text-[18px] font-bold text-gray-800 mb-1">Reviews
                                    ({{ $product->reviews->where('status', 1)->count() }})</h2>
                                <p class="text-[13px] text-gray-500">Get specific details about this product from customers
                                    who own it.</p>
                                @if ($product->reviews->where('status', 1)->count() > 0)
                                    ...
                                    <div class="text-[#f59e0b] text-[14px] mt-2 mb-1 flex items-center gap-1">
                                        ...
                                        @endfor
                                        <span class="text-gray-800 font-bold text-[14px] ml-1.5">{{ $avgRating }} out
                                            of 5</span>
                                    </div>
                                @endif
                            </div>
                            @auth
                                ...
                                @if ($hasPurchased && !$alreadyReviewed)
                                    <button onclick="toggleReviewModal()"
                                        class="shrink-0 border border-gray-800 text-gray-800 py-2 px-4 rounded text-[13px] font-semibold transition-colors hover:bg-gray-800 hover:text-white">Write
                                        a Review</button>
                                @elseif($alreadyReviewed)
                                    <span class="text-xs font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full"><i
                                            class="fas fa-check-circle mr-1"></i> You reviewed this</span>
                                @else
                                    <p class="text-[11px] text-gray-400 italic">Only verified buyers can review</p>
                                @endif
                            @else
                                <a href="{{ route('login') }}"
                                    class="shrink-0 border border-gray-800 text-gray-800 py-2 px-4 rounded text-[13px] font-semibold transition-colors hover:bg-gray-800 hover:text-white">Login
                                    to Review</a>
                            @endauth
                        </div>
                        ...
                        @forelse($product->reviews->where('status', 1) as $review)
                            <div class="py-4 border-b border-[#f2f4f8] last:border-b-0">
                                ...
                                <div class="text-[12px] text-gray-400 font-medium">By {{ $review->user->name }} on
                        ...
                        <div
                            ...
                            <div class="p-8">
                                <form action="{{ route('product.review.store') }}" method="POST" class="space-y-6">
                                    ...
                                    <div>
                                        <label
                                            class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 text-center">Your
                                            Rating</label>
                                        <div class="flex justify-center gap-3 text-3xl text-gray-300">
                                            <input type="hidden" name="rating" id="selectedRating" value="5">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <i class="fas fa-star cursor-pointer hover:text-accent-orange transition-colors star-rating-btn text-accent-orange"
                                                    data-rating="{{ $i }}"
                                    ...
                                    <div>
                                        <label
                                            class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Your
                                            Experience</label>
                                        <textarea name="review" rows="4" required placeholder="What did you like or dislike? How was the performance?"
                                            class="w-full bg-gray-50 border border-gray-200 rounded-lg py-3 px-4 focus:ring-2 focus:ring-accent-blue outline-none transition-all text-sm"></textarea>
                                    </div>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-5">
                            ...
                            <div>
                                @foreach ($relatedProducts as $related)
                                    <div
                                        class="flex gap-4 items-center text-left pb-4 mb-4 border-b border-gray-100 last:border-b-0 last:mb-0 last:pb-0 group">
                                        <img src="{{ $related->thumbnail ? asset('storage/' . $related->thumbnail) : 'https://placehold.co/100x100/f9fafb/a3a3a3?text=No+Image' }}"
                                            alt="{{ $related->name }}"
                                            class="w-[70px] h-[70px] object-contain shrink-0 group-hover:scale-105 transition-transform duration-300">
                                        <div>
                                            <h4 class="text-[13px] text-gray-800 leading-snug mb-1.5"><a
                                                    href="{{ url('product/' . $related->slug) }}"
                                                    class="no-underline text-gray-800 transition-colors hover:text-accent-orange">{{ Str::limit($related->name, 40) }}</a>
                                            </h4>
                                            <div class="text-[14px] font-bold text-accent-orange mb-2">
                                                @if ($related->discount_price && $related->discount_price < $related->price)
                                                    {{ number_format($related->discount_price, 0) }}৳ <span
                                                        class="line-through text-gray-400 font-normal text-[11px] ml-1">{{ number_format($related->price, 0) }}৳</span>
                                                @else
    ...
    <div id="quotationModal"
        ...
        <div class="bg-white rounded shadow-xl w-full max-w-[800px] overflow-hidden relative mx-4 relative">
            <button onclick="toggleQuotationModal()"
                class="absolute top-2 right-2 text-white hover:text-gray-300 focus:outline-none z-10 bg-black rounded-full w-5 h-5 flex items-center justify-center text-[10px]">
                ...
                <form action="{{ route('quotation.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 mb-4">
                        <div class="md:col-span-2">
                            <input type="text" name="name" required placeholder="Enter your Name"
                                class="border border-gray-200 rounded-sm px-3 py-2 text-[13px] w-full focus:outline-none focus:border-accent-blue placeholder-gray-400">
                        </div>
                        <input type="text" name="phone" required placeholder="Contact No"
                            class="border border-gray-200 rounded-sm px-3 py-2 text-[13px] w-full focus:outline-none focus:border-accent-blue placeholder-gray-400">
                    ...
                    <div class="mb-6 text-center">
                        ...
                        <input type="file" name="attachment"
                            class="text-[13px] text-gray-600 file:mr-2 file:py-1 file:px-2 file:rounded-sm file:border file:border-gray-300 file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 w-full md:w-auto">
                    </div>

                    <div class="flex justify-center gap-4 border-t border-gray-100 pt-5">
                        <button type="submit"
                            class="bg-blue-900 text-white px-8 py-2 rounded-sm text-[13px] font-bold hover:bg-blue-950 transition-colors">Submit
                            Request</button>
                        <button type="button" onclick="toggleQuotationModal()"
                            class="bg-gray-400 text-white px-8 py-2 rounded-sm text-[13px] font-bold hover:bg-gray-500 transition-colors">Close</button>
                    </div>
                </form>
            </div>
        </div>
